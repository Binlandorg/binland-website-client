...
export const HeroText = styled.div<{ $heightDiff: number }>`
	--fix-size: min(
		${(props) => Math.round(props.$heightDiff / 2)}px,
		var(--size)
	);
		...
		& .title {
			...
			& .border-radius-left-bottom {
				...
				&::after {
					top: 0;
					left: 0;
					box-shadow: calc(var(--size) * -0.5) calc(var(--size) * -0.5) 0 0px
						var(--color);
				}
			}
			...
			& .border-radius-right-bottom {
				...
				&::after {
					top: 0;
					left: 0;
					box-shadow: calc(var(--fix-size) * 0.5) calc(var(--fix-size) * -0.5) 0 0px
						var(--color);
				}
			}
		}
