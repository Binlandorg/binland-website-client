...
export const SelectWrapper = styled.div<{$width: number | undefined, $open: boolean}>`
	position: relative;
    cursor: pointer;
    `

export const Label = styled.div<{$width: number | undefined, $open: boolean, $isError: boolean}>`
    --border-color: ${({$isError}) => $isError ? colors.primary.normal[500] : colors.tertiary.focus[500]};
    display: flex;
    ...
    ${({$open}) => $open ? `box-shadow: 
            0 -2px 0 0 var(--border-color),
            -2px 0 0 0 var(--border-color),
            2px 0 0 0 var(--border-color);` : ""}

    ${({$isError}) => $isError ? `outline: 2px solid var(--border-color);` : ""}
    position: relative;
...
export const Options = styled.div<{$width: number | undefined, $open: boolean, $isError: boolean}>`
    --border-color: ${({$isError}) => $isError ? colors.primary.normal[500] : colors.tertiary.focus[500]};
    display: flex;
