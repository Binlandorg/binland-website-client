...
export const SelectContainer = styled.div<{ $open: boolean, $isError: boolean }>`
    --color: ${({ $isError }) =>($isError ? colors.primary.normal[500] : colors.tertiary.focus[500])};
    ...
    ${({ $isError }) =>($isError ? `outline: 2px solid ${colors.primary.normal[500]};` : "")}
